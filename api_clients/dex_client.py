# api_clients/dex_client.py
# Professional async DexScreener API client with rate limiting, caching, and retry logic
...
RATE_LIMITS_RPM = {
    "slow": 60,   # boosts, profiles, takeovers
    "fast": 300,  # pairs, search, tokens
}
...
class SlidingWindowLimiter:
    """Async sliding window rate limiter (requests per minute)."""
...
class DexScreenerClientAsync:
    """
    Async HTTP client for the DexScreener API.

    Features:
    - Two-bucket rate limiting (slow: 60 RPM, fast: 300 RPM)
    - TTL-based in-memory cache
    - Retry with exponential backoff + jitter
    - 429/5xx cooldown with adaptive penalty
    - Path segment validation to prevent traversal attacks
    """

    _CACHE_MAX_SIZE = 500  # Max entries; oldest are evicted when exceeded
    ...
    async def __aenter__(self) -> DexScreenerClientAsync:
        return self
    ...
    async def _cache_set(self, key: str, payload: Any) -> None:
        async with self._cache_lock:
            ...
            # If still over size limit, evict oldest (smallest expiry)
            if len(self._cache) > self._CACHE_MAX_SIZE:
                overflow = len(self._cache) - self._CACHE_MAX_SIZE
    ...
    async def _record_bucket_cooldown(self, bucket: str, retry_after: float | None) -> None:
        async with self._bucket_state_lock:
            ...
            self._bucket_pause_until[bucket] = max(
                self._bucket_pause_until.get(bucket, 0.0),
                monotonic() + cooldown,
            )

    async def _decay_bucket_penalty(self, bucket: str) -> None:
        async with self._bucket_state_lock:
            self._bucket_penalty_seconds[bucket] = max(
                self._bucket_penalty_seconds.get(bucket, 0.0) * 0.65, 0.0
            )
        ...
        while True:
            ...
            try:
                ...
                if attempt < MAX_RETRIES:
                    ...
                    sleep_for = RETRY_BACKOFF_SECONDS * (2 ** attempt) + random.uniform(0.02, 0.2)
    ...
    async def get_pairs_for_tokens(
        self, chain_id: str, token_addresses: list[str]
    ) -> list[dict[str, Any]]:
        """Batch fetch pairs for up to 30 token addresses per request."""
        _validate_path_segment(chain_id, "chain_id")
