# api_clients/dex_scanner.py
# HotScanner: multi-dimensional DexScreener token scanner with advanced analytics
...
from .dex_client import DexScreenerClientAsync
from .dex_models import CandidateAnalytics, HotTokenCandidate, PairSnapshot
from .dex_scoring import score_hotness_detail
...
class ScanFilters:
    """Filters controlling which tokens the HotScanner returns."""
    chains: tuple[str, ...]
    limit: int = 20
...
class HotScanner:
    """
    Multi-source DexScreener hot token scanner.

    Pipeline:
    1. Collect seed tokens from boosts, profiles, takeovers, and trend searches
    2. Batch-fetch pair data for seeds
    3. Score each candidate with 8-component hotness scoring
    4. Enrich with compression/breakout/momentum/risk analytics
    5. Rank and return top N candidates
    """

    def __init__(self, client: DexScreenerClientAsync) -> None:
        self._client = client
    ...
    def _velocity_components(pair: PairSnapshot) -> tuple[float, float]:
        vol_baseline = (
            (pair.volume_h6 - pair.volume_h1) / 5.0
            if pair.volume_h6 > pair.volume_h1
            else (pair.volume_h24 / 24.0)
        )
        ...
        tx_baseline = (
            (tx_h24 - pair.txns_h1) / 23.0
            if tx_h24 > pair.txns_h1
            else (tx_h24 / 24.0)
        )
    ...
    def _compression_and_readiness(
        ...
        *,
        ...
        flow_build = self._clip(
            (min(volume_velocity, 3.0) / 3.0 + min(txn_velocity, 3.0) / 3.0) / 2.0,
            0.0,
            1.0,
        )
        pressure = self._clip((buy_pressure + 0.15) / 0.85, 0.0, 1.0)

        compression_score = self._clip(
            compression_price * 0.55 + flow_build * 0.30 + pressure * 0.15, 0.0, 1.0
        )
    ...
    def _boost_velocity(
        self, key: tuple[str, str], boost_total: float, now_s: float
    ) -> float:
    ...
    def _momentum_metrics(
        ...
        fast_decay = (
            half_life_min is not None
            and half_life_min <= 12.0
            and decay_ratio <= 0.45
        )
    ...
    def _prune_histories(self, now_s: float) -> None:
        ...
        stale_boost_keys = [
            key for key, (ts, _value) in self._boost_history.items() if ts < cutoff
        ]
        ...
        if len(self._boost_history) > self._max_history_keys:
            for key, _value in sorted(
                self._boost_history.items(), key=lambda item: item[1][0]
            )[: -self._max_history_keys]:
                self._boost_history.pop(key, None)
    ...
    def _enrich_candidates(self, candidates: list[HotTokenCandidate]) -> None:
        ...
        chain_baseline_h1 = {
            chain: median(values)
            for chain, values in chain_momentum.items()
            if values
        }
        chain_baseline_velocity = {
            chain: median(values)
            for chain, values in chain_velocity.items()
            if values
        }
        ...
        for candidate in candidates:
            ...
            relative_strength = (pair.price_change_h1 - baseline_h1) + (
                (volume_velocity - baseline_velocity) * 5.0
            )
            ...
            half_life_min, decay_ratio, fast_decay = self._momentum_metrics(
                key, pair.price_change_h1, now_s
            )
            candidate.analytics = CandidateAnalytics(
                ...
                momentum_half_life_min=(
                    round(half_life_min, 2) if half_life_min is not None else None
                ),
                momentum_decay_ratio=(
                    round(decay_ratio, 3) if decay_ratio is not None else None
                ),
            ...
            if (
                ...
                and volume_velocity >= 1.15
                and txn_velocity >= 1.15
            ):
            ...
            if fast_decay:
                ...
                half_life_min is not None
                and half_life_min >= 25
                and (decay_ratio or 0.0) > 0.65
            ):
    ...
    async def _collect_seeds(
        ...
        for row in profiles:
            ...
            if chain_id not in chains or not token:
                continue
            upsert(chain_id, token, has_profile=True, discovery="profiles")
        ...
        # Search-based discovery to supplement boosts for each chain
        ...
        search_rows_batches = await asyncio.gather(
            *(self._client.search_pairs(query) for query in _TRENDING_QUERIES),
        ...
        for batch in search_rows_batches:
            ...
            for row in batch:
                ...
                tx_h1 = (
                    row.get("txns", {}).get("h1", {})
                    if isinstance(row.get("txns"), dict)
                    else {}
                )
                ...
                if pair_created_at:
                    age_h = max(
                        (now_ms - as_int(pair_created_at)) / 3_600_000.0, 0.0
                    )
    ...
    def _best_pair_from_rows(
        self, rows: list[dict[str, Any]]
    ) -> dict[tuple[str, str], PairSnapshot]:
    ...
    async def _prefetch_pairs_for_seeds(
        self, seeds: list[_SeedToken]
    ) -> dict[tuple[str, str], PairSnapshot]:
        ...
        for chain_id, token_addresses in by_chain.items():
            try:
                ...
                logger.warning(f"Failed to prefetch pairs for {chain_id}: {exc}")
                continue
            aggregated_rows.extend(rows)

        return self._best_pair_from_rows(aggregated_rows)

    async def _best_pair_for_token(
    ...
    async def scan(self, filters: ScanFilters) -> list[HotTokenCandidate]:
        """
        Full scan pipeline: seed → fetch pairs → score → enrich → rank.

        Returns top `filters.limit` HotTokenCandidate objects sorted by score.
        """
        ...
        async def worker(seed: _SeedToken) -> None:
            async with semaphore:
                ...
                if pair is None:
                    pair = await self._best_pair_for_token(
                        seed.chain_id, seed.token_address
                    )
                if not pair:
                    return
                ...
                analytics = CandidateAnalytics(
                    base_score=score, score_components=components
                )
                results.append(
                    HotTokenCandidate(
                        pair=pair,
        ...
        # De-duplicate per token, keep highest score
    ...
    async def search(self, query: str, limit: int = 20) -> list[PairSnapshot]:
        ...
        return [PairSnapshot.from_api(row) for row in rows[:limit]]
...
def run_hot_scan(
    ...
    """
    Synchronous wrapper around HotScanner.scan().

    Always runs in a dedicated background thread with its own event loop so
    it is safe to call from both sync code and from inside a running event loop
    (e.g. Flask request handlers, ThreadPoolExecutor workers).

    Returns a list of HotTokenCandidate sorted by score (highest first).
    """
    if filters is None:
        filters = ScanFilters(
            chains=chains,
    ...
    async def _run() -> list[HotTokenCandidate]:
    ...
    # Always dispatch to a thread so we never conflict with a caller's event loop.
