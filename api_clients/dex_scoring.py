# api_clients/dex_scoring.py
# Hotness scoring engine for DexScreener token candidates
...
from .dex_models import HotTokenCandidate, PairSnapshot
...
def score_hotness_detail(
    ...
    """
    8-component composite hotness score (0-100 scale).

    Components:
    - Volume (30pts): log-scaled 24h volume vs $7.5M baseline
    - Transactions (20pts): log-scaled 1h txns vs 4K baseline
    - Liquidity (18pts): log-scaled USD liquidity vs $3M baseline
    - Momentum (12pts): 1h price change normalised
    - Buy pressure (8pts): buy/sell ratio
    - Boost factor (7pts): log-scaled DexScreener boost total
    - Recency (3pts): pair age bonus
    - Profile listing (2pts): has DexScreener profile
    """
...
def build_distribution_heuristics(candidate: HotTokenCandidate) -> dict[str, float | str]:
    """Analyse token distribution health: liquidity/mcap ratios, volume/liquidity metrics."""
    pair = candidate.pair
