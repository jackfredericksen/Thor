...
class DexscreenerClient(BaseAPIClient):
    ...
    def get_pairs_for_token(self, token_address: str) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Fetched {len(processed_pairs)} pairs for token {token_address}"
            )
    ...
    def fetch_token_details(
        ...
        try:
            ...
            best_pair = max(
                pairs, key=lambda p: safe_float(p.get("liquidity", {}).get("usd", 0))
            )
    ...
    def _process_token_data(self, raw_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            txns_24h = safe_int(txns.get("h24", {}).get("buys", 0)) + safe_int(
            ...
            processed_data = {
                ...
                "liquidity_to_mcap_ratio": liquidity_usd / market_cap
                if market_cap > 0
                else 0,
