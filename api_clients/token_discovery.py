...
class TokenDiscovery:
    ...
    def discover_all_tokens(self, max_workers: int = 5) -> List[Dict[str, Any]]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_source = {
                executor.submit(
                    self._fetch_from_source, source_name, source
                ): source_name
    ...
    def _fetch_from_source(
        self, source_name: str, source: TokenSource
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            api_key = None
            source_base = source_name.split("_")[
                0
            ]  # e.g., 'birdeye_trending' -> 'birdeye'
    ...
    def _make_request(
        self, url: str, headers: Dict = None, api_key: str = None
    ) -> requests.Response:
    ...
    def _parse_pumpfun(self, data: List, max_tokens: int) -> List[Dict[str, Any]]:
        ...
        for item in data[:max_tokens]:
            ...
            token = {
                ...
                "price_usd": float(item.get("usd_market_cap", 0))
                / max(float(item.get("total_supply", 1)), 1),
        ...
        for address, info in price_data.items():
            ...
            token = {
                ...
                "price_change_24h": float(
                    extra.get("quotedPrice", {}).get("24hChange", 0)
                ),
                "source_raw": "jupiter",
            }
