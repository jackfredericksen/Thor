...
class Config:
    ...
    def should_process_token(self, token_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if market_cap > 0:
                mcap_ok = (
                    self.FILTERS['MIN_MARKET_CAP'] <= market_cap <= 
    ...
    def validate_config(self) -> List[str]:
        ...
        if self.TRADING['MAX_POSITION_SIZE_USD'] < self.TRADING['MIN_POSITION_SIZE_USD']:
...
if __name__ == "__main__":
    ...
    if issues:
        ...
        print(f"📊 Configured for {len(config.DISCOVERY['SOURCE_PRIORITIES'])} discovery sources")
