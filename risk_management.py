...
class RiskManager:
    ...
    def validate_trade(
        self, token_address: str, side: str, quantity: float, price: float
    ) -> Tuple[bool, str]:
        """
        ...
        with self.lock:
            ...
            if side == "buy":
                ...
                if new_position_value > self.max_position_size:
                    return (
                        False,
                        f"Position size limit exceeded: ${new_position_value:,.0f} > ${self.max_position_size:,.0f}",
                    )
                ...
                if new_exposure > self.max_total_exposure:
                    return (
                        False,
                        f"Total exposure limit exceeded: ${new_exposure:,.0f} > ${self.max_total_exposure:,.0f}",
                    )
                ...
                if portfolio_value > 0:
                    ...
                    if new_position_pct > 0.25:  # Max 25% in single position
                        return (
                            False,
                            f"Concentration risk: {new_position_pct:.1%} > 25%",
                        )
            ...
            # Check if selling more than we have
                ...
                if quantity > current_position.quantity:
                    return (
                        False,
                        f"Insufficient position: {current_position.quantity} < {quantity}",
                    )

            return True, "Trade validated"

    def calculate_position_size(
        self, token_address: str, price: float, confidence_score: float = 1.0
    ) -> float:
        """
        ...
        max_allowed_usd = min(
            self.max_position_size, self.max_total_exposure - self.get_total_exposure()
        )
    ...
    def add_position(
        self, token_address: str, symbol: str, quantity: float, price: float
    ) -> None:
        ...
        with self.lock:
            ...
            if token_address in self.positions:
                ...
                logger.info(
                    f"Updated position {symbol}: {total_quantity} @ ${new_avg_price:.6f}"
                )
            else:
                # Create new position
    ...
    def reduce_position(
        self, token_address: str, quantity: float, exit_price: float
    ) -> Optional[float]:
        """
        ...
        with self.lock:
            if token_address not in self.positions:
                logger.warning(
                    f"Attempted to reduce non-existent position: {token_address}"
                )
                return None
            ...
            if quantity > position.quantity:
                logger.error(
                    f"Cannot reduce position by {quantity}, only have {position.quantity}"
                )
                return None
            ...
            if position.quantity <= 0:
                ...
                logger.info(
                    f"Closed position {position.symbol} with PnL: ${realized_pnl:.2f}"
                )
            else:
                logger.info(
                    f"Reduced position {position.symbol} by {quantity}, PnL: ${realized_pnl:.2f}"
                )

            # Record trade
            self._record_trade(
                token_address, "sell", quantity, exit_price, realized_pnl
            )
    ...
    def get_risk_metrics(self) -> RiskMetrics:
        ...
        with self.lock:
            ...
            largest_position_pct = (
                largest_position_value / portfolio_value if portfolio_value > 0 else 0
            )
    ...
    def _calculate_win_rate(self) -> float:
        ...
        winning_trades = sum(
            1 for trade in self.trade_history if trade.get("realized_pnl", 0) > 0
        )
