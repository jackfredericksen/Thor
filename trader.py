...
class Trader:
    ...
    def _execute_buy(
        ...
        try:
            ...
            logger.info(
                f"Executing BUY: {quantity:.6f} {symbol} @ ${price:.6f} (${trade_value:.2f})"
            )

            return self._paper_trade_buy(
                token_address, symbol, quantity, price, token_info
            )
    ...
    def _execute_sell(
        ...
        try:
            ...
            logger.info(
                f"Executing SELL: {quantity:.6f} {symbol} @ ${price:.6f} ({reason})"
            )

            return self._paper_trade_sell(
                token_address, symbol, quantity, price, reason
            )
    ...
    def _paper_trade_buy(
        ...
        try:
            ...
            logger.info(
                f"Paper trade BUY completed: {quantity:.6f} {symbol} @ ${price:.6f}"
            )
            return True
    ...
    def monitor_order(
        self, order_id: str, symbol: str = "Unknown", timeout: int = 300
    ) -> bool:
