...
def handle_api_response(response: requests.Response, service_name: str = "API") -> dict:
    ...
    try:
        ...
        if status_code == 429:
            ...
            logger.warning(
                f"{service_name} rate limited. Retry after {retry_after} seconds"
            )
            raise RateLimitException(
                f"Rate limited by {service_name}. Retry after {retry_after}s"
            )
            ...
            logger.error(
                f"{service_name} client error: {status_code} - {response.text}"
            )
