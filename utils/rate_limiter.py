    ...
    def get_limiter(self, service: str, requests_per_minute: int) -> RateLimiter:
        ...
        with self.lock:
            if service not in self.limiters:
                ...
                logger.debug(f"Created rate limiter for {service}: {requests_per_minute} req/min")
            return self.limiters[service]
    
    def acquire(self, service: str, requests_per_minute: int, 
                tokens: int = 1, timeout: Optional[float] = 30.0) -> bool:
        ...
        if success:
            ...
            if elapsed > 1.0:  # Log if we waited more than 1 second
                logger.info(f"Rate limited {service}: waited {elapsed:.2f}s for {tokens} tokens")
        else:
...
def rate_limited(service: str, requests_per_minute: int, tokens: int = 1, timeout: float = 30.0):
    """
...
class RequestTracker:
    ...
    def record_request(self, service: str, success: bool = True, 
        ...
        with self.lock:
            ...
            while self.requests[service] and self.requests[service][0]['timestamp'] < cutoff:
    ...
    def get_stats(self, service: str) -> Dict:
        ...
        response_times = [r['response_time'] for r in requests if r['response_time'] is not None]
